...
import { generateBoard } from '../board/board-generator'
import { processAction, ProcessResult } from '../processors'
...
export class GameFlowManager {
  ...
  getCurrentPlayer(): Player {
    ...
    if (!player) {
      throw new Error(`Current player ${this.state.currentPlayer} not found in game state`)
    }
    return player
  }
  ...
  getPlayersInOrder(): Player[] {
    ...
    return playerIds.map(id => {
      ...
      if (!player) {
        throw new Error(`Player ${id} not found in game state`)
      }
      return player
    })
  }
  ...
  getNextPlayer(): Player {
    ...
    if (!nextPlayer) {
      throw new Error(`Next player ${playerIds[nextIndex]} not found in game state`)
    }
    return nextPlayer
  }
  ...
  private static deserializeState(data: {
    id: string
    phase: GamePhase
    turn: number
    currentPlayer: string
