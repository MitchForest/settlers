...
import { Result, ResultUtils, GameStateError, InvalidActionError, ErrorContextBuilder } from '../errors'
...
export class StateMachineValidator {
  ...
  validateTurnOrder(
    ...
    try {
      ...
      return ResultUtils.success(isValidTransition)
    } catch (error) {
      return ResultUtils.failure(
        new InvalidActionError(
  ...
  private validateSetupTurnOrder(
    ...
    if (gameState.phase === 'setup1') {
      ...
      const expectedNextIndex = (currentIndex + 1) % playerIds.length
      return ResultUtils.success(nextIndex === expectedNextIndex)
    }
