...
import { GameState, GameAction, GameEvent, PlayerId, Player } from '../types'
import { calculateLongestRoad } from '../core/placement-rules'
import { GAME_RULES, VICTORY_POINTS } from '../constants'
...
export abstract class BaseActionProcessor<T extends GameAction> implements ActionProcessor<T> {
  ...
  protected createEvent(type: string, playerId: PlayerId | undefined, data: Record<string, unknown> = {}): GameEvent {
...
export class StateManager {
  ...
  updatePlayer(state: GameState, playerId: PlayerId, updates: Partial<Player>): GameState {
...
export class EventFactory {
  ...
  create(type: string, playerId: PlayerId | undefined, data: Record<string, unknown> = {}): GameEvent {
    return {
...
export class LongestRoadUpdater implements PostProcessor {
  process(state: GameState): GameState {
    ...
    // Calculate longest road for each player using imported function
